...
int main(int argc, char const *argv[]){
	...
	for(;;){
		...
		pthread_create(&tid,NULL,poolWorker,&s);
		pthread_detach(tid);
...
void verify(double *sharedMem,double *sharedMemSolution,struct send data,int *flag){
	...
	fprintf(out, "\nBy SVD Method \n");
		...
		send(data.socket,sendToClients,(1+sizeof(double)*(data.rows + PROCESS)*data.columns),0);
...
void *poolWorker(void *argm){
	...
	int t=0;
		...
		for (;;){
			...
			if (num==0)
				break;
			recv(t,&data,sizeof(struct send),0);
