...
struct msg{
	long type;
	char mat[sizeof(struct numOfMatches)];
...
int countInDir(char *str,const char *str1,FILE *out,int fd,int fileDescr, int msgId);
...
int main(int argc, char *argv[]){
	...
	while (((fd = open(CASCADE, O_RDWR)) == -1) && (errno == EINTR)); 
	...
	if (argc != 3){
		...
		return 1;
		 ...
		 error = countInDir(argv[2],argv[1],out,fd,fileDescr,msgId);
	}
	...
	temp1=1;
	while(temp1 !=-1){
		temp1 = msgrcv(msgId, &tempMsg,sizeof(numMat), 0, 0 | IPC_NOWAIT);
		
		if(temp1!=-1){
			memcpy(&numMat,tempMsg.mat,sizeof(numMat));
			num += numMat.matches;
			lines += numMat.lines;
			filesNo += numMat.numOfFiles;
		}				
	}
    ...
    printf("Number of shared memory %d and their sizes : ",filesNo);
    for(i=0;i<count+1;i++){
    	printf("%lu ",sizeof(struct numOfMatches));
    }
...
int countInDir(char *str,const char *str1,FILE *out,int fd, int fileDescr,int msgId){
	...
	argm.str2[0] = '\0';
	argm.str1[0] = '\0';
	argArr= malloc(sizeof(argm)*value);

	write(fd,&value,sizeof(value));

	if((shmId = shmget(IPC_PRIVATE,sizeof(struct numOfMatches),IPC_CREAT | 0644)) < 0) {
	...
	while ((direntp = readdir(dirp))!= NULL){
		...
		if(direntp->d_name[0]!='~' && (strcmp(direntp->d_name,"..")!=0) && (strcmp(direntp->d_name,".")!=0)){
			...
			if(isDirectory(cwd) && (strcmp(direntp->d_name,"..")!=0) && (strcmp(direntp->d_name,".")!=0)){
				...
				if(pid == 0){
					...
					countInDir(str,str1,out,fd,fileDescr,msgId);
					close(fdes[0]);
					close(fdes[1]);
					fclose(out);
					exit(1);
		...
		free(argArr);
		msgToSend.type=1;
		memcpy(msgToSend.mat,shm,sizeof(msgToSend.mat));

		msgsnd(msgId,(void *)&msgToSend,sizeof(msgToSend.mat), 0);
...
void *wrapper(void *argm){
	...
	num = counter(arg->str,arg->str2,arg->str1,arg->out,&line);
	
	arg->numMatch->matches += num;
	arg->numMatch->lines += line;
	arg->numMatch->numOfFiles +=1;
